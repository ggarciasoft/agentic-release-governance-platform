...
- When attaching PRs, always include `mergeCommitId` set to `lastMergeSourceCommit.commitId`
  from the Azure DevOps PR response. This is required for correct deployment pipeline correlation.
  If the field is absent on a PR (e.g. still active), pass `null`.
